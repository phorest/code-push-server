...
export class AppInsights {
  ...
  private static SERVICE_RESOURCE_DEFINITIONS: ServiceResourceDefinition[] = [
    ...
    // /accessKeys/def123
    {
      resource: ServiceResource.AccessKeysWithId,
      regExp: /^\/accessKeys\/[^\/]+[\/]?$/i,
      tag: 'AccessKey',
    },
    // /account
    ...
    {
      resource: ServiceResource.AppTransfer,
      regExp: /^\/apps\/[^\/]+\/transfer\/[^\/]+[\/]?$/i,
      tag: 'App transfer',
    },
    // /apps
    ...
    {
      resource: ServiceResource.Collaborators,
      regExp: /^\/apps\/[^\/]+\/collaborators[\/]?$/i,
      tag: 'Collaborators',
    },
    // /apps/abc123/collaborators/foo@bar.com
    {
      resource: ServiceResource.CollaboratorsWithEmail,
      regExp: /^\/apps\/[^\/]+\/collaborators\/[^\/]+[\/]?$/i,
      tag: 'Collaborator',
    },
    ...
    {
      resource: ServiceResource.Deployments,
      regExp: /^\/apps\/[^\/]+\/deployments[\/]?$/i,
      tag: 'Deployments',
    },
    // /apps/abc123/deployments/xyz123
    {
      resource: ServiceResource.DeploymentsWithId,
      regExp: /^\/apps\/[^\/]+\/deployments\/[^\/]+[\/]?$/i,
      tag: 'Deployment',
    },
    // /auth/link/github
    {
      resource: ServiceResource.LinkGitHub,
      regExp: /^\/auth\/link\/github[\/]?/i,
      tag: 'Link GitHub account',
    },
    // /auth/link/microsoft
    {
      resource: ServiceResource.LinkMicrosoft,
      regExp: /^\/auth\/link\/microsoft[\/]?/i,
      tag: 'Link Microsoft account',
    },
    // /auth/login/github
    {
      resource: ServiceResource.LoginGitHub,
      regExp: /^\/auth\/login\/github[\/]?/i,
      tag: 'Login with GitHub',
    },
    // /auth/login/microsoft
    {
      resource: ServiceResource.LoginMicrosoft,
      regExp: /^\/auth\/login\/microsoft[\/]?/i,
      tag: 'Login with Microsoft',
    },
    ...
    {
      resource: ServiceResource.Promote,
      regExp: /^\/apps\/[^\/]+\/deployments\/[^\/]+\/promote\/[^\/]+[\/]?$/i,
      tag: 'Package',
    },
    // /auth/register/github
    {
      resource: ServiceResource.RegisterGitHub,
      regExp: /^\/auth\/register\/github[\/]?/i,
      tag: 'Register with GitHub',
    },
    // /auth/register/microsoft
    {
      resource: ServiceResource.RegisterMicrosoft,
      regExp: /^\/auth\/register\/microsoft[\/]?/i,
      tag: 'Register with Microsoft',
    },
    ...
    {
      resource: ServiceResource.ReportStatusDeploy,
      regExp: /^\/reportStatus\/deploy[\/]?$/i,
      tag: 'ReportStatusDeploy',
    },
    // /reportStatus/download or /reportStatus/download/
    {
      resource: ServiceResource.ReportStatusDownload,
      regExp: /^\/reportStatus\/download[\/]?$/i,
      tag: 'ReportStatusDownload',
    },
    // /apps/abc123/deployments/xyz123/rollback or /apps/abc123/deployments/xyz123/rollback/v4
    {
      resource: ServiceResource.Rollback,
      regExp: /^\/apps\/[^\/]+\/deployments\/[^\/]+\/rollback(\/[^\/]+)?[\/]?$/i,
      tag: 'Package',
    },
    ...
    router.use((req: express.Request, res: express.Response, next: (err?: Error) => void): any => {
      ...
      if (property) {
        ...
        if (isUpdateCheck) {
          ...
          if (req.body) {
            const info: restTypes.PackageInfo =
              tryJSON(req.body.packageInfo) || req.body.packageInfo
  ...
  private getTagProperty(
    ...
    if (
      tag.indexOf('Link') < 0 &&
      tag.indexOf('Login') < 0 &&
      tag.indexOf('Logout') < 0 &&
      tag.indexOf('Register') < 0
    ) {
      switch (method) {
        case 'GET':
