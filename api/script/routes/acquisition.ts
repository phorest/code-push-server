...
function createResponseUsingStorage(
  ...
  if (isMissingPatchVersion) {
    ...
    if (semverTagIndex === -1) {
      ...
      updateRequest.appVersion =
        originalAppVersion.slice(0, semverTagIndex) +
        '.0' +
        originalAppVersion.slice(semverTagIndex)
    }
  }
  ...
  if (validationUtils.isValidUpdateCheckRequest(updateRequest)) {
    return storage
      ...
      .then((packageHistory: storageTypes.Package[]) => {
        const updateObject: UpdateCheckCacheResponse = acquisitionUtils.getUpdatePackageInfo(
          packageHistory,
          updateRequest,
        )
        if (
          (isMissingPatchVersion || isPlainIntegerNumber) &&
          updateObject.originalPackage.appVersion === updateRequest.appVersion
        ) {
...
export function getHealthRouter(config: AcquisitionConfig): express.Router {
  ...
  router.get(
    '/health',
    (req: express.Request, res: express.Response, next: (err?: any) => void): any => {
      storage
        .checkHealth()
        .then(() => {
...
export function getAcquisitionRouter(config: AcquisitionConfig): express.Router {
  ...
  const reportStatusDeploy = function(
    ...
    const previousDeploymentKey =
      req.body.previousDeploymentKey || req.body.previous_deployment_key || deploymentKey
    const previousLabelOrAppVersion =
      req.body.previousLabelOrAppVersion || req.body.previous_label_or_app_version
    ...
    if (semver.valid(sdkVersion) && semver.gte(sdkVersion, METRICS_BREAKING_VERSION)) {
      ...
      if (req.body.label && req.body.status === redis.DEPLOYMENT_FAILED) {
        redisUpdatePromise = redisManager.incrementLabelStatusCount(
          deploymentKey,
          req.body.label,
          req.body.status,
        )
      ...
      return redisManager
        ...
        .then((currentVersionLabel: string) => {
          if (req.body.label && req.body.label !== currentVersionLabel) {
            return redisManager
              .incrementLabelStatusCount(deploymentKey, req.body.label, req.body.status)
              .then(() => {
                if (req.body.status === redis.DEPLOYMENT_SUCCEEDED) {
                  return redisManager.updateActiveAppForClient(
                    deploymentKey,
                    clientUniqueId,
                    req.body.label,
                    currentVersionLabel,
