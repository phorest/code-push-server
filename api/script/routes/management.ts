...
export function getManagementRouter(config: ManagementConfig): Router {
  ...
  router.post('/accessKeys', (req: Request, res: Response, next: (err?: any) => void): any => {
    const accountId: string = req.user.id
    const accessKeyRequest: restTypes.AccessKeyRequest = converterUtils.accessKeyRequestFromBody(
      req.body,
    )
    ...
    const validationErrors: validationUtils.ValidationError[] =
      validationUtils.validateAccessKeyRequest(accessKeyRequest, /*isUpdate=*/ false)
    ...
    const accessKey: restTypes.AccessKey = <restTypes.AccessKey>(
      (<restTypes.AccessKey>accessKeyRequest)
    )
      ...
      .then((accessKeys: storageTypes.AccessKey[]): void | Promise<void> => {
        if (NameResolver.isDuplicate(accessKeys, accessKey.name)) {
          ...
          errorUtils.sendConflictError(
            res,
            `The access key "${accessKey.friendlyName}" already exists.`,
          )
          return
        }

        const storageAccessKey: storageTypes.AccessKey =
          converterUtils.toStorageAccessKey(accessKey)
  ...
  router.patch(
    ...
    (req: Request, res: Response, next: (err?: any) => void): any => {
      ...
      const validationErrors: validationUtils.ValidationError[] =
        validationUtils.validateAccessKeyRequest(accessKeyRequest, /*isUpdate=*/ true)
      if (validationErrors.length) {
        res.status(400).send(validationErrors)
        return
      }
        ...
        .then((accessKeys: storageTypes.AccessKey[]): Promise<void> => {
          ...
          if (!updatedAccessKey) {
            throw errorUtils.restError(
              errorUtils.ErrorCode.NotFound,
              `The access key "${accessKeyName}" does not exist.`,
            )
          }
      ...
      storage
        ...
        .then((accessKeys: storageTypes.AccessKey[]) => {
          ...
          if (accessKeyDeletionPromises.length) {
            ...
            throw errorUtils.restError(
              errorUtils.ErrorCode.NotFound,
              `There are no sessions associated with "${createdBy}."`,
            )
          }
        })
        .then((): void => {
    ...
    storage
      ...
      .then((apps: storageTypes.App[]) => {
        const restAppPromises: Promise<restTypes.App>[] = apps.map((app: storageTypes.App) => {
          return storage
            .getDeployments(accountId, app.id)
            .then((deployments: storageTypes.Deployment[]) => {
              const deploymentNames: string[] = deployments.map(
                (deployment: storageTypes.Deployment) => deployment.name,
              )
  ...
  router.post('/apps', (req: Request, res: Response, next: (err?: any) => void): any => {
    const accountId: string = req.user.id
    const appRequest: restTypes.AppCreationRequest = converterUtils.appCreationRequestFromBody(
      req.body,
    )
    ...
    if (validationErrors.length) {
      ...
      storage
        ...
        .then((apps: storageTypes.App[]): void | Promise<void> => {
          ...
          let storageApp: storageTypes.App = converterUtils.toStorageApp(
            appRequest,
            new Date().getTime(),
          )
          ...
          return storage
            ...
            .then((app: storageTypes.App): Promise<string[]> => {
              ...
              if (!appRequest.manuallyProvisionDeployments) {
                ...
                const deploymentPromises: Promise<string>[] = defaultDeployments.map(
            ...
            .then((deploymentNames: string[]): void => {
              ...
              res.status(201).send({
                app: converterUtils.toRestApp(
                  storageApp,
                  /*displayName=*/ storageApp.name,
                  deploymentNames,
                ),
              })
            })
  ...
  router.get('/apps/:appName', (req: Request, res: Response, next: (err?: any) => void): any => {
    ...
    nameResolver
      ...
      .then((deployments: storageTypes.Deployment[]) => {
        ...
        res.send({
          app: converterUtils.toRestApp(storageApp, /*displayName=*/ appName, deploymentNames),
        })
      })
      .catch((error: error.CodePushError) => errorUtils.restErrorHandler(res, error, next))
      .done()
  ...
  router.delete('/apps/:appName', (req: Request, res: Response, next: (err?: any) => void): any => {
    ...
    nameResolver
      ...
      .then((deployments: storageTypes.Deployment[]) => {
        const invalidationPromises: Promise<void>[] = deployments.map(
          (deployment: storageTypes.Deployment) => {
      ...
      .then((apps: storageTypes.App[]): void | Promise<void> => {
        ...
        if (validationErrors.length) {
          ...
          return storage
            ...
            .then(() => {
              return storage
                .getDeployments(accountId, existingApp.id)
                .then((deployments: storageTypes.Deployment[]) => {
                  const deploymentNames: string[] = deployments.map(
                    (deployment: storageTypes.Deployment) => {
                      return deployment.name
      ...
      nameResolver
        ...
        .then((app: storageTypes.App) => {
          const isAttemptingToRemoveSelf: boolean =
            ...
            app.collaborators[email].isCurrentAccount
          throwIfInvalidPermissions(
            app,
            isAttemptingToRemoveSelf
              ? storageTypes.Permissions.Collaborator
              : storageTypes.Permissions.Owner,
          )
  ...
  router.post(
    ...
    (req: Request, res: Response, next: (err?: any) => void): any => {
      ...
      const validationErrors = validationUtils.validateDeployment(
        restDeployment,
        /*isUpdate=*/ false,
      )
      if (validationErrors.length) {
      ...
      const storageDeployment: storageTypes.Deployment = converterUtils.toStorageDeployment(
        restDeployment,
        new Date().getTime(),
      )
      nameResolver
        ...
        .then((deployments: storageTypes.Deployment[]): void | Promise<void> => {
          ...
          return storage
            .addDeployment(accountId, appId, storageDeployment)
            .then((deploymentId: string): void => {
              restDeployment = converterUtils.toRestDeployment(storageDeployment)
              res.setHeader(
                'Location',
                urlEncode([`/apps/${appName}/deployments/${restDeployment.name}`]),
              )
    ...
    (req: Request, res: Response, next: (err?: any) => void): any => {
      ...
      nameResolver
        ...
        .then((storageDeployments: storageTypes.Deployment[]): void | Promise<void> => {
          const storageDeployment: storageTypes.Deployment = NameResolver.findByName(
            storageDeployments,
            deploymentName,
          )
          ...
          if (
            (restDeployment.name || restDeployment.name === '') &&
            restDeployment.name !== storageDeployment.name
          ) {
  ...
  router.patch(
    ...
    (req: Request, res: Response, next: (err?: any) => void): any => {
      ...
      const validationErrors: validationUtils.ValidationError[] =
        validationUtils.validatePackageInfo(info, /*allOptional*/ true)
      if (validationErrors.length) {
        errorUtils.sendMalformedRequestError(res, JSON.stringify(validationErrors))
        return
      }
      ...
      nameResolver
        ...
        .then((storageDeployments: storageTypes.Deployment[]) => {
          ...
          if (!storageDeployment) {
            throw errorUtils.restError(
              errorUtils.ErrorCode.NotFound,
              `Deployment "${deploymentName}" does not exist.`,
            )
          }
        ...
        .then((packageHistory: storageTypes.Package[]) => {
          ...
          if (!packageToUpdate) {
            throw errorUtils.restError(
              errorUtils.ErrorCode.NotFound,
              'Release not found for given label.',
            )
          }

          const newIsDisabled: boolean = info.isDisabled
          if (
            validationUtils.isDefined(newIsDisabled) &&
          ...
          if (
            validationUtils.isDefined(newIsMandatory) &&
          ...
          if (updateRelease) {
            return storage
              .updatePackageHistory(accountId, appId, storageDeployment.id, packageHistory)
              .then(() => {
  ...
  router.post(
    ...
    (req: Request, res: Response, next: (err?: any) => void): any => {
      ...
      const validationErrors: validationUtils.ValidationError[] =
        validationUtils.validatePackageInfo(restPackage, /*allOptional*/ false)
      if (validationErrors.length) {
        errorUtils.sendMalformedRequestError(res, JSON.stringify(validationErrors))
        return
      }
      ...
      fs.stat(filePath, (err: NodeJS.ErrnoException, stats: fs.Stats): void => {
        ...
        nameResolver
          ...
          .then((deployment: storageTypes.Deployment) => {
            ...
            if (
              existingPackage &&
              isUnfinishedRollout(existingPackage.rollout) &&
              !existingPackage.isDisabled
            ) {
              throw errorUtils.restError(
                errorUtils.ErrorCode.Conflict,
          ...
          .then((history: storageTypes.Package[]) => {
            lastPackageHashWithSameAppVersion = getLastPackageHashWithSameAppVersion(
              history,
              restPackage.appVersion,
            )
      ...
      if (!redisManager.isEnabled) {
        ...
        nameResolver
          ...
          .then((metrics: redis.DeploymentMetrics) => {
            const deploymentMetrics: restTypes.DeploymentMetrics =
              converterUtils.toRestDeploymentMetrics(metrics)
  ...
  router.post(
    ...
    (req: Request, res: Response, next: (err?: any) => void): any => {
      ...
      nameResolver
        ...
        .spread(
          (
            sourceDeployment: storageTypes.Deployment,
            ...
            if (info.label) {
              return storage
                .getPackageHistory(accountId, appId, sourceDeployment.id)
                .then((sourceHistory: storageTypes.Package[]) => {
        ...
        .then(() => {
          ...
          if (!sourcePackage) {
            throw errorUtils.restError(
              errorUtils.ErrorCode.NotFound,
              'Cannot promote from a deployment with no enabled releases.',
            )
          } else if (
            validationUtils.isDefined(info.rollout) &&
            !validationUtils.isValidRolloutField(info.rollout)
          ) {
            throw errorUtils.restError(
            ...
            destPackage &&
            isUnfinishedRollout(destPackage.rollout) &&
            !destPackage.isDisabled
          ) {
            throw errorUtils.restError(
              errorUtils.ErrorCode.Conflict,
        ...
        .then((destHistory: storageTypes.Package[]) => {
          if (
            ...
            getLastPackageHashWithSameAppVersion(destHistory, sourcePackage.appVersion)
          ) {
            throw errorUtils.restError(
              errorUtils.ErrorCode.Conflict,
          ...
          const isMandatory: boolean = validationUtils.isDefined(info.isMandatory)
            ? info.isMandatory
            : sourcePackage.isMandatory
          const newPackage: storageTypes.Package = {
            ...
            isDisabled: validationUtils.isDefined(info.isDisabled)
              ? info.isDisabled
              : sourcePackage.isDisabled,
    ...
    (req: Request, res: Response, next: (err?: any) => void): any => {
      ...
      nameResolver
        ...
        .then((packageHistory: storageTypes.Package[]) => {
          ...
          return storage
            .commitPackage(accountId, appId, deploymentToRollback.id, newPackage)
            .then((): Promise<void> => {
    ...
    if (collaboratorsMap) {
      for (const email of Object.keys(collaboratorsMap)) {
        if ((<storageTypes.CollaboratorProperties>collaboratorsMap[email]).isCurrentAccount) {
          ...
          isPermitted =
            permission === storageTypes.Permissions.Owner || permission === requiredPermission
          break
        }
      }
    }
