...
export class PassportAuthentication {
  ...
  public getRouter(): Router {
    ...
    const isGitHubAuthenticationEnabled: boolean =
      !!this._serverUrl && !!gitHubClientId && !!gitHubClientSecret
    ...
    const isMicrosoftAuthenticationEnabled: boolean =
      !!this._serverUrl && !!microsoftClientId && !!microsoftClientSecret
      ...
      (req: Request, res: Response, next: (err?: any) => void): any => {
        ...
        if (!emailAddress && providerName === PassportAuthentication.MICROSOFT_PROVIDER_NAME) {
          ...
          restErrorUtils.sendUnknownError(
            res,
            new Error(
              `Couldn't get an email address from the ${providerName} OAuth provider for user ${JSON.stringify(
                user,
              )}`,
            ),
            next,
        ...
        this._storageInstance
          ...
          .then(
            (account: storage.Account): void | Promise<void> => {
              const existingProviderId: string = PassportAuthentication.getProviderId(
                account,
                providerName,
              )
              ...
              switch (action) {
                ...
                case 'link':
                  if (existingProviderId) {
                    restErrorUtils.sendAlreadyExistsPage(
                      res,
                      'You are already registered with the service using this provider.',
                    )
                    return
                  }
                ...
                case 'login':
                  if (!isProviderValid) {
                    restErrorUtils.sendForbiddenPage(
                      res,
                      'You are not registered with the service using this provider account.',
                    )
                    return
                  }
