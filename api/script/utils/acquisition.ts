...
export function getUpdatePackageInfo(
  ...
  const updatePackage: UpdatePackage = getUpdatePackage(
    packageHistory,
    request,
    /*ignoreRolloutPackages*/ false,
  )
  let cacheResponse: UpdateCheckCacheResponse

  if (isUnfinishedRollout(updatePackage.rollout)) {
    const origUpdatePackage: UpdatePackage = getUpdatePackage(
      packageHistory,
      request,
      /*ignoreRolloutPackages*/ true,
    )
...
function getUpdatePackage(
  ...
  for (let i = packageHistory.length - 1; i >= 0; i--) {
    ...
    if (
      packageEntry.isDisabled ||
      (ignoreRolloutPackages && isUnfinishedRollout(packageEntry.rollout))
    ) {
      continue
    }
  ...
  if (!latestEnabledPackage) {
    ...
    updateDetails.shouldRunBinaryVersion ||
    latestSatisfyingEnabledPackage.packageHash === request.packageHash
  ) {
    ...
    updateDetails.packageSize =
      latestSatisfyingEnabledPackage.diffPackageMap[request.packageHash].size
  } else {
