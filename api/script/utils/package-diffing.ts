...
export class PackageDiffer {
  ...
  public generateDiffPackageMap(
    ...
    if (!newPackage || !newPackage.blobUrl || !newPackage.manifestBlobUrl) {
      return q.reject<storageTypes.PackageHashToBlobInfoMap>(
        diffErrorUtils.diffError(
          diffErrorUtils.ErrorCode.InvalidArguments,
    ...
    const historyPromise: Promise<storageTypes.Package[]> = this._storage.getPackageHistory(
      accountId,
      appId,
      deploymentId,
    )
  ...
  public generateDiffArchive(
    oldManifest: PackageManifest,
    newManifest: PackageManifest,
    newArchiveFilePath: string,
  ): Promise<string> {
    return Promise<string>(
      (
        ...
        const diffFilePath = path.join(
          ...
          'diff_' + PackageDiffer.randomString(20) + '.zip',
        )
          ...
          yauzl.open(newArchiveFilePath, (error?: any, zipFile?: yauzl.ZipFile): void => {
            ...
            zipFile
              ...
              .on('entry', (entry: yauzl.IEntry): void => {
                if (
                  !PackageDiffer.isEntryInMap(
                    entry.fileName,
                    /*hash*/ null,
                    diff.newOrUpdatedEntries,
  ...
  private getManifest(appPackage: storageTypes.Package): Promise<PackageManifest> {
    return Promise<PackageManifest>(
      (
        resolve: (manifest: PackageManifest) => void,
  ...
  private getPackagesToDiff(
    ...
    for (let i = history.length - 1; i >= 0; i--) {
      ...
      const isMatchingAppVersion: boolean = PackageDiffer.isMatchingAppVersion(
        appVersion,
        history[i].appVersion,
      )
  ...
  private static isMatchingAppVersion(baseAppVersion: string, newAppVersion: string): boolean {
    ...
    if (!semver.valid(baseAppVersion)) {
      ...
      if (!semver.valid(newAppVersion)) {
        ...
        isMatchingAppVersion =
          semver.validRange(newAppVersion) === semver.validRange(baseAppVersion)
      } else {
