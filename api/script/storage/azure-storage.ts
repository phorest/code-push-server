...
module Keys {
  ...
  export function generateHierarchicalAppKey(
    markLeaf: boolean,
    appId: string,
    deploymentId?: string,
  ): string {
  ...
  export function generateHierarchicalAccountKey(
    markLeaf: boolean,
    accountId: string,
    appId?: string,
  ): string {
  ...
  export function getShortcutAccessKeyPartitionKey(
    ...
    return delimit(
      'accessKey',
      hash ? utils.hashWithSHA256(accessKeyName) : accessKeyName,
    ...
    parameters.forEach((parameter: string): void => {
      if (parameter && (parameter.indexOf(DELIMITER) >= 0 || parameter.indexOf(LEAF_MARKER) >= 0)) {
        throw storage.storageError(
          ...
          `The parameter '${parameter}' contained invalid characters.`,
        )
      }
    })
  }
...
export class AzureStorage implements storage.Storage {
    ...
    return q.Promise<void>((resolve, reject) => {
      this._setupPromise
        .then(() => {
                ...
                if ((<any>entity).health !== 'health') {
                  tableReject(
                    storage.storageError(
                      ...
                      'The Azure Tables service failed the health check',
                    ),
                  )
                } else {
                  tableResolve()
          ...
          const acquisitionBlobCheck: q.Promise<void> = this.blobHealthCheck(
            AzureStorage.TABLE_NAME,
          )
          const historyBlobCheck: q.Promise<void> = this.blobHealthCheck(
  ...
  public addAccount(account: storage.Account): q.Promise<string> {
    ...
    return this._setupPromise
      .then(() => {
        const entity: any = this.wrap(
          account,
          emailShortcutAddress.partitionKeyPointer,
          emailShortcutAddress.rowKeyPointer,
        )
      ...
      .then(() => {
        const entity: any = this.wrap(
          accountPointer,
          hierarchicalAddress.partitionKeyPointer,
          hierarchicalAddress.rowKeyPointer,
        )
  ...
  public getApp(
    accountId: string,
    appId: string,
    keepCollaboratorIds: boolean = false,
  ): q.Promise<storage.App> {
    return this._setupPromise
      .then(() => {
  ...
  public transferApp(accountId: string, appId: string, email: string): q.Promise<void> {
    ...
    return this._setupPromise
      .then(() => {
        const getAppPromise: q.Promise<storage.App> = this.getApp(
          accountId,
          appId,
          /*keepCollaboratorIds*/ true,
        )
      ...
      .spread((appPromiseResult: storage.App, accountPromiseResult: storage.Account) => {
        ...
        requestingCollaboratorEmail = AzureStorage.getEmailForAccountId(
          app.collaborators,
          accountId,
        )
        ...
        if (requestingCollaboratorEmail === email) {
          throw storage.storageError(
            ...
            'The given account already owns the app.',
          )
        }
      ...
      .then((appsForCollaborator: storage.App[]) => {
        if (storage.NameResolver.isDuplicate(appsForCollaborator, app.name)) {
          throw storage.storageError(
            storage.ErrorCode.AlreadyExists,
            'Cannot transfer ownership. An app with name "' +
              app.name +
              '" already exists for the given collaborator.',
          )
        }
        ...
        AzureStorage.setCollaboratorPermission(
          app.collaborators,
          requestingCollaboratorEmail,
          storage.Permissions.Collaborator,
        )
      ...
      .then((app: storage.App) => {
        ...
        if (!removedCollabProperties) {
          throw storage.storageError(
            ...
            'The given email is not a collaborator for this app.',
          )
        }
  ...
  public addDeployment(
    ...
    return this._setupPromise
      ...
      .then(() => {
        const shortcutPartitionKey: string = Keys.getShortcutDeploymentKeyPartitionKey(
          deployment.key,
        )
  ...
  public updatePackageHistory(
    ...
    if (!history || !history.length) {
      throw storage.storageError(
        storage.ErrorCode.Invalid,
        'Cannot clear package history from an update operation',
      )
    }
    ...
    return this._setupPromise
      ...
      .then((buffer) => {
        return this._blobService
          ...
          .uploadBlockBlob(blobId, buffer, buffer.byteLength)
      })
      .then(() => {
        return blobId
      })
  ...
  public getBlobUrl(blobId: string): q.Promise<string> {
    return this._setupPromise
      .then(() => {
        return this._blobService.getContainerClient(AzureStorage.TABLE_NAME).getBlobClient(blobId)
          .url
      })
      .catch(AzureStorage.azureErrorHandler)
  }
  ...
  public removeAccessKey(accountId: string, accessKeyId: string): q.Promise<void> {
    return this._setupPromise
      ...
      .then((accessKey) => {
        const partitionKey: string = Keys.getAccountPartitionKey(accountId)
        const rowKey: string = Keys.getAccessKeyRowKey(accountId, accessKeyId)
        const shortcutAccessKeyPartitionKey: string = Keys.getShortcutAccessKeyPartitionKey(
          accessKey.name,
          false,
        )
  ...
  private setup(accountName?: string, accountKey?: string): q.Promise<void> {
    ...
    const tableHealthEntity: any = this.wrap(
      { health: 'health' },
      /*partitionKey=*/ 'health',
      /*rowKey=*/ 'health',
    )
  ...
  private updateAppWithPermission(
    accountId: string,
    app: storage.App,
    updateCollaborator: boolean = false,
  ): q.Promise<void> {
    const appId: string = app.id
    if (!appId) throw new Error('No app id')
  ...
  private async cleanUpByAppHierarchy(appId: string, deploymentId?: string): Promise<void> {
    const partitionKey: string = Keys.getAppPartitionKey(appId)
    const rowKey: string = Keys.getHierarchicalAppRowKey(appId, deploymentId)
    const descendantsSearchKey: string = Keys.generateHierarchicalAppKey(
      /*markLeaf=*/ false,
      appId,
      deploymentId,
    )
  ...
  private static unflattenApp(flatApp: any, currentAccountId: string): storage.App {
    ...
    const currentUserEmail: string = AzureStorage.getEmailForAccountId(
      flatApp.collaborators,
      currentAccountId,
    )
  ...
  private static isOwner(collaboratorsMap: storage.CollaboratorMap, email: string): boolean {
    return (
      ...
      (<storage.CollaboratorProperties>collaboratorsMap[email]).permission ===
        storage.Permissions.Owner
    )
  }
  ...
  private static setCollaboratorPermission(
  ...
  private static getEmailForAccountId(
    collaboratorsMap: storage.CollaboratorMap,
    accountId: string,
  ): string {
    if (collaboratorsMap) {
