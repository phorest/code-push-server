...
export class NameResolver {
  ...
  private static findAppByName(apps: App[], displayName: string): App {
    ...
    if (ownerEmail) {
      for (let i = 0; i < candidates.length; i++) {
        ...
        if (
          app.collaborators[ownerEmail] &&
          app.collaborators[ownerEmail].permission === Permissions.Owner
        ) {
          return app
        }
      }
  ...
  private static errorMessageOverride(
    code: ErrorCode,
    message: string,
  ...
  public resolveAccessKey(accountId: string, name: string): Promise<AccessKey> {
    return this._storage
      ...
      .catch(
        NameResolver.errorMessageOverride(
          ErrorCode.NotFound,
          `Access key "${name}" does not exist.`,
        ),
      )
  }
