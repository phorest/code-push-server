...
function storageTests(
  StorageType: new (...args: any[]) => storageTypes.Storage,
      ...
      return storage.checkHealth().then(
        /*returnedHealthy*/ () => {
          assert.equal(
            StorageType,
            AzureStorage,
            'Should only return healthy if running Azure storage',
          )
        },
        /*returnedUnhealthy*/ () => {
          assert.equal(
            StorageType,
            JsonStorage,
            'Should only return unhealthy if running JSON storage',
          )
        },
      )
    })
  ...
  describe('Account', () => {
    ...
    it('will reject promise for a non-existent email', () => {
      return storage
        .getAccountByEmail('non-existent-emaiL@test.com')
        .then(failOnCallSucceeded, (error: storageTypes.StorageError) => {
          assert.equal(error.code, storageTypes.ErrorCode.NotFound)
        })
    })
  ...
  describe('Deployment', () => {
    ...
    it('rejects promise with an invalid deploymentId', () => {
      return storage
        .getDeployment(account.id, app.id, 'invalid')
        .then(failOnCallSucceeded, (error: storageTypes.StorageError) => {
          assert.equal(error.code, storageTypes.ErrorCode.NotFound)
        })
    })
    ...
    it('rejects promise when retrieving by invalid app', () => {
      ...
      return storage
        .getDeployments(account.id, 'invalid')
        .then(failOnCallSucceeded, (error: storageTypes.StorageError) => {
          assert.equal(error.code, storageTypes.ErrorCode.NotFound)
        })
    })
    ...
    it('rejects promise when removing a non-existent deployment', () => {
      return storage
        .removeDeployment(account.id, app.id, 'invalid')
        .then(failOnCallSucceeded, (error: storageTypes.StorageError) => {
          assert.equal(error.code, storageTypes.ErrorCode.NotFound)
        })
    })
    ...
    it('will reject promise when updating non-existent entry', () => {
      ...
      return storage
        .updateDeployment(account.id, app.id, deployment)
        .then(failOnCallSucceeded, (error: storageTypes.StorageError) => {
          assert.equal(error.code, storageTypes.ErrorCode.NotFound)
        })
    })
  ...
  describe('Package', () => {
    ...
    it('can get empty package', () => {
      return storage
        .getDeployment(account.id, app.id, deployment.id)
        .then((deployment: storageTypes.Deployment) => {
    ...
    it('can add and get a package', () => {
      ...
      return storage
        ...
        .then((deploymentPackages: storageTypes.Package[]) => {
          assert.equal(
            'description123',
            deploymentPackages[deploymentPackages.length - 1].description,
          )
        })
    })
        ...
        for (var i = 1; i <= 3; i++) {
          promiseChain = promiseChain
            ...
            .then((committedPackage: storageTypes.Package) => {
              var lastPackage: storageTypes.Package =
                expectedPackageHistory[expectedPackageHistory.length - 1]
      ...
      it('can get package history', () => {
        return storage
          ...
          .then((actualPackageHistory: storageTypes.Package[]) => {
            assert.equal(
              JSON.stringify(actualPackageHistory),
              JSON.stringify(expectedPackageHistory),
            )
          })
      })
      ...
      it('can update package history', () => {
        return storage
          ...
          .then((actualPackageHistory: storageTypes.Package[]) => {
            ...
            return storage.updatePackageHistory(
              account.id,
              app.id,
              deployment.id,
              expectedPackageHistory,
