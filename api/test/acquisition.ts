...
describe('Acquisition Rest API', () => {
  ...
  before((): q.Promise<void> => {
    var useJsonStorage: boolean =
      !process.env.TEST_AZURE_STORAGE && !process.env.AZURE_ACQUISITION_URL
    ...
    return q<void>(null)
      ...
      .then(() => {
        ...
        appPackage.diffPackageMap = {
          [previousPackageHash]: { url: 'http://www.server.com/diffBlob2', size: 3 },
        }
        appPackage.description = 'Test package for 1.0.0'
      ...
      .then(() => {
        ...
        appPackage.diffPackageMap = {
          [previousPackageHash]: { url: 'http://www.server.com/diffBlob3', size: 5 },
        }
        appPackage.description = 'Test package for 1.0.0'
        appPackage.isMandatory = false
  ...
  describe('Get /health', () => {
    it('should be healthy if and only if correctly configured', (done) => {
      var isProductionReady: boolean =
        storageInstance instanceof AzureStorage && redisManager && redisManager.isEnabled
    ...
    it('returns 200 and available optional update with diff blob URL', (done) => {
      ...
      request(server || serverUrl)
        ...
        .end(function (err: any, result: any) {
          ...
          assert.equal(response.updateInfo.isAvailable, true)
          assert.equal(
            response.updateInfo.downloadURL,
            appPackage.diffPackageMap[previousPackageHash].url,
          )
      ...
      before(() => {
        ...
        return storageInstance
          ...
          .then((deploymentId: string) => {
            ...
            return storageInstance.commitPackage(
              account2.id,
              app2.id,
              deployment2.id,
              deployment2.package,
      ...
      before(() => {
        ...
        return storageInstance
          ...
          .then((packageHistory: storage.Package[]) => {
            ...
            return storageInstance.updatePackageHistory(
              account2.id,
              app2.id,
              deployment2.id,
              packageHistory,
            ...
            if (redisManager.isEnabled) {
              return redisManager
                ...
                .then((metrics: any) => {
                  ...
                  assert.equal(
                    metrics[redis.Utilities.getLabelStatusField('v2', redis.DEPLOYMENT_SUCCEEDED)],
                    1,
                  )
                  assert.equal(
                    metrics[redis.Utilities.getLabelStatusField('v3', redis.DEPLOYMENT_FAILED)],
                    1,
                  )
                  done()
                })
            ...
            if (redisManager.isEnabled) {
              return redisManager
                ...
                .then((metrics: any) => {
                  ...
                  assert.equal(
                    metrics[redis.Utilities.getLabelStatusField('v1', redis.DEPLOYMENT_SUCCEEDED)],
                    1,
                  )
                  done()
                })
          ...
          .end((err: any, result: any): void => {
            ...
            if (redisManager.isEnabled) {
              redisManager
                ...
                .then((metrics: any) => {
                  assert.equal(
                    metrics[redis.Utilities.getLabelStatusField('v2', redis.DOWNLOADED)],
                    1,
                  )
                  done()
                })
